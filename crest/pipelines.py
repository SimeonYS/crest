...
class CrestPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('crest.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `crest`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM crest WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO crest (date, title, link, content)"
